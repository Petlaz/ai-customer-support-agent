...
Langfuse: every LLM call is traced automatically via a LangChain
CallbackHandler linked to the workflow trace ID.
"""
import json
import logging
...
def classify_ticket_node(state: AgentState) -> dict:
    ...
    # Langfuse callback handler (no-op if not configured)
